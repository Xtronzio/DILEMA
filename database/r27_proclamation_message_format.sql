-- R27 · Formato breve de proclamas; conserva acceso solo a destinatarios.
...
set text='ANÓNIMO: '||substring(text from length('PROCLAMA ANÓNIMA PARA LA MESA · ')+1)
where text like 'PROCLAMA ANÓNIMA PARA LA MESA · %';
update public.debate_private_proclamations
set text=regexp_replace(text,'^PROCLAMA DE (.*?) PARA .* · ', E'\\1: ')
where text like 'PROCLAMA DE % PARA % · %';
